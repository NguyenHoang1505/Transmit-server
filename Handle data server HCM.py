...
files=glob('*.json')
Json_new=open("Data_all_HCM.json", "w",encoding='utf-8')
for file in files:
    with open(file) as f:
        for line in f:
            Json_new.write(line)
...
data = []
with open('Data_all_HCM.json') as f:
    for line in f:
...
file=open("Data_HCM_Handle.json", "w",encoding='utf-8')
for i in range(data_handle.shape[0]):
...
data_handle.to_csv("Data_HCM_Handle.csv",encoding='utf-8')
